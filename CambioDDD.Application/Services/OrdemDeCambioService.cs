...
namespace CambioDDD.Application.Services
{
    public class OrdemDeCambioService : IOrdemDeCambioService
    {
        ...
        public void Adicionar(decimal valorOperacao, Moeda moedaOrigem, Moeda moedaDestino)
        {
            if (valorOperacao <= 0)
                throw new ArgumentException("Erro - Valor da ordem de câmbio deve ser maior que zero.");
            OrdemDeCambio ordemDeCambio = new OrdemDeCambio(Guid.NewGuid(), valorOperacao, moedaOrigem, moedaDestino);
